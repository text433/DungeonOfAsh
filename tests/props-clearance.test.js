...
const start=source.indexOf('    createColumn(tileX, tileY)');
...
const column=scene.createColumn(10,10);
assert.equal(column.body.w,16);
assert.equal(column.body.h,48);
assert.equal(column.y-48+column.body.y,8*16,'Column blocks its top tile');
assert.equal(column.y-48+column.body.y+column.body.h,11*16,'Column blocks through its bottom tile');
console.log('Column collision covers all three rendered tiles.');
