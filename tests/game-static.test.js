...
if (!html.includes('<script src="map-rules.js?v=61"></script>')) throw new Error("HTML neielādē jaunākos kartes noteikumus");
if (!html.includes('<script src="progression.js?v=28"></script>')) throw new Error("HTML neielādē progresa sistēmu");
if (!html.includes('<script src="game.js?v=61"></script>')) throw new Error("HTML neielādē jaunāko spēles kodu");
if (html.includes('id="minimap-label"') || html.includes('KARTE · SIENAS')) {
  throw new Error("Minikartē joprojām redzams sienu teksts");
}
if (!html.includes('<link rel="stylesheet" href="style.css?v=61" />')) throw new Error("HTML neielādē jaunāko HUD noformējumu");
...
if (!minimapSource.includes("floorBoundsCells.concat(wallBoundsCells)")) {
  throw new Error("Minikartes robežās nav iekļautas ārējās sienas");
}
